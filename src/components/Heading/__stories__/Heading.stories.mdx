...
However, the import statement `{Heading}` from 'monday-ui-core' is already reserved for the <Link href="/?path=/docs/text-legacyheading--overview">LegacyHeading</Link> component.
Therefore, the new Heading with the updated API can be accessed using the following import statement:

```javascript
import { Heading } from "monday-ui-core/next";
```
